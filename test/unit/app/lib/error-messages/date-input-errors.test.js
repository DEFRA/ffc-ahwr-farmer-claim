...
describe('date input error message', () => {
  ...
  test('returns message to enter value when no values have been entered', () => {
    ...
    expect(res.errorMessage.text).toEqual('Enter the date of review')
  })

  test.each([
    { label: labels.day, value: 31, error: 'The date of review must include a month and a year' },
    { label: labels.month, value: 12, error: 'The date of review must include a day and a year' },
    { label: labels.year, value: 2022, error: 'The date of review must include a day and a month' }
  ...
  test.each([
    { label: labels.day, error: 'The date of review must include a day' },
    { label: labels.month, error: 'The date of review must include a month' },
    { label: labels.year, error: 'The date of review must include a year' }
  ...
  test.each([
    ...
    expect(errorMessage.text).toEqual('The date of review must be a real date')
  ...
  test.each([
    ...
    { label: labels.year, value: 999, expectedErrorMessage: `The date of review must be the same or after ${firstValidDate.toLocaleString('en-GB', { day: 'numeric', month: 'long', year: 'numeric' })} when you accepted your agreement offer` }
