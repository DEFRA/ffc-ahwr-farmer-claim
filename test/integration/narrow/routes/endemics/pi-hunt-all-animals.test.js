...
describe('PI Hunt recommended tests', () => {
  ...
  describe(`GET ${url} route`, () => {
    test.each([
      ...
      getEndemicsClaim.mockImplementation(() => ({
        reference: 'TEMP-6GSE-PIR8', typeOfLivestock, reviewTestResults, latestEndemicsApplication: {}
      }))
  ...
  describe(`POST ${url} route`, () => {
    ...
    test('shows error when payload is invalid', async () => {
      ...
      expect($('#main-content > div > div > div > div > div > ul > li > a').text()).toMatch('Select yes if the PI hunt was done on all beef cattle in the herd')
    })
  })
})
