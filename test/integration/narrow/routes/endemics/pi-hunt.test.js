...
describe('Species numbers test', () => {
  ...
  describe(`GET ${url} route`, () => {
    test('returns 200', async () => {
      ...
      expect($('.govuk-fieldset__heading').text().trim()).toEqual('Did the vet do a persistently infected (PI) hunt for bovine viral diarrhoea (BVD) on all animals in the herd?')
  ...
  describe(`POST ${url} route`, () => {
    ...
    test('shows error when payload is invalid', async () => {
      ...
      expect(res.statusCode).toBe(400)
      const $ = cheerio.load(res.payload)
      expect($('h1').text().trim()).toMatch('Did the vet do a persistently infected (PI) hunt for bovine viral diarrhoea (BVD) on all animals in the herd?')
