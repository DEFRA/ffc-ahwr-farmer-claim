...
describe('Vet, enter date of visit', () => {
  ...
  describe(`POST to ${url} route`, () => {
    ...
    test.each([
      { description: 'visit before application - application created today, visit date yesterday', day: yesterday.getDate(), month: yesterday.getMonth(), year: yesterday.getFullYear(), errorMessage: `Date must be the same or after ${new Date(today).toLocaleString('en-GB', { day: 'numeric', month: 'long', year: 'numeric' })} when the application was made`, errorHighlights: allErrorHighlights, applicationCreationDate: today },
