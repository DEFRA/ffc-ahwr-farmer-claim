...
// page element
const VETVISIT_SAME_AS_REVIEW_RADIO='#whenTestingWasCarriedOut'
const VETVISIT_DIFFERENT_FROM_REVIEW_RADIO='#whenTestingWasCarriedOut-2'
...
const CONTENT = '.govuk-summary-list__value'
...
class StartPageActions extends CommonActions {
  ...
  async VerifyError_PastDate(){
    await this.clickOn(VETVISIT_DIFFERENT_FROM_REVIEW_RADIO)
    const currentDate = new Date();
...
async clickonSameDay(){
  await this.clickOn(VETVISIT_SAME_AS_REVIEW_RADIO)
}
